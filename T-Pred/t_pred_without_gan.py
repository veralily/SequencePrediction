...
class T_Pred(object):
	...
	def loss_with_wasserstein(self, pred_e, pred_t, real_e, real_t, input_t, sample_t, sample_target_t):
		...
		return gen_train_op, disc_train_op, gen_cost, disc_cost, gen_t_cost, gen_e_cost, disc_cost_1, gradient_penalty
	...
	def build(self):
		...
		hidden_r = self.encoder(self.inputs_e, self.inputs_t)
		self.pred_e = pred_e = self.g_event(hidden_r)
		self.pred_t = pred_t = self.g_time(hidden_r)

		gen_train_op, disc_train_op, gen_cost, disc_cost, gen_t_cost, gen_e_cost, disc_cost_1, gradient_penalty = self.loss_with_wasserstein(
			pred_e,
			pred_t,
			self.targets_e,
			self.targets_t,
		...
		self.deviation = tf.reduce_mean(tf.abs(self.target_t - tf.squeeze(pred_t)),[0,1])
	...
	def train(self, sess, args):
		...
		for epoch in range(args.iters):
			...
			for i in range(iterations):
				...
				# 	train_e, train_target_e, train_t, train_target_t = train_data_iter.next()
				# 	_, _, sample_t, sample_target_t = sample_train_data_iter.next()
				# 	feed_dict = {
				# 	self.input_e : train_e,
				# 	self.inputs_t : train_t,
				# 	self.target_t : train_target_t,
				# 	self.targets_e : train_target_e,
				# 	self.sample_t : sample_t,
				# 	self.sample_target_t : sample_target_t}
