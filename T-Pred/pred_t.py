...
class T_Pred(object):
	...
	def encoder(self, cell_type, inputs):
		'''
	...
	def loss_with_wasserstein(self, pred_t, real_t, input_t, sample_t, sample_target_t):
		...
		log_t_loss = tf.losses.log_loss(pred_t, real_t)
		...
		return gen_train_op, disc_train_op, gen_cost, disc_cost, disc_cost_1, gradient_penalty
	...
	def build(self):
		...
		inputs_t = tf.expand_dims(self.inputs_t, 2)
		hidden_r = self.encoder(self.cell_type, inputs_t)
		self.pred_t = pred_t = self.g_time(hidden_r)

		gen_train_op, disc_train_op, gen_cost, disc_cost, disc_cost_1, gradient_penalty = self.loss_with_wasserstein(
			pred_t,
			self.targets_t,
			self.inputs_t,
			self.sample_t,
	...
	def train(self, sess, args):
		...
		for epoch in range(args.iters):
			...
			for i in range(iterations):
				...
				_, deviation, pred_t, d_loss, g_loss, disc_cost_1, gradient_penalty = sess.run(
					[self.g_train_op, self.deviation, self.pred_t, self.d_cost, self.g_cost, self.disc_cost_1, self.gradient_penalty],
					feed_dict = feed_dict)
				...
				if i % (iterations // 10) == 0:
					print('[epoch: %d, %d] deviation: %f, d_loss: %f, g_loss: %f' %(
						epoch,
						int(i // (iterations // 10)),
						...
						g_loss))
					print()
					print('d_1_loss: %f, g_penal_loss: %f' %(
						disc_cost_1,
						gradient_penalty))
					print()
			...
			for i in range(iterations):
				...
				deviation, pred_t, d_loss, g_loss, disc_cost_1, gradient_penalty = sess.run(
					[self.deviation, self.pred_t, self.d_cost, self.g_cost, self.disc_cost_1, self.gradient_penalty],
					feed_dict = feed_dict)
				...
				if i % (iterations // 10) == 0:
					print('%f, deviation: %f, d_loss: %f, g_loss: %f' %(
						i // (iterations // 10),
						sum_deviation / (sum_iter * self.batch_size),
						d_loss,
						g_loss))
