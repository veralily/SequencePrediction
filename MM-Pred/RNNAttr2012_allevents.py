...
flags.DEFINE_string("traindir", "./train/2012_new", "Directory to save checkpoints and gnuplot files.")
...
resultFile = open("2012/e1_ResultFile_attention_allevents" + time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime()) + ".logging.csv", 'w')
...
with tf.Graph().as_default() as G:
        ...
        for batchNum, (c, x, y) in enumerate(
            ...
            if (epochNumBatches > 10):
                if batchNum % (epochNumBatches // 10) == 10:
                    ...
                    # print("variable_b: [%s]" %(' '.join([str(num) for num in v_b])))
                    # print("variable_b_a1: [%s]" %(' '.join([str(num) for num in v_b_a1])))  
    ...
    for batchNum, (c, x, y) in enumerate(read.words_iterator(input_validation_traces, target_validation_traces, FLAGS.batch_size, FLAGS.num_unroll_step)):
        v_b, v_b_a1, batchNumCorrPred, batchCrossEnt, batchCost, state, _ = sess.run(
            [weight_b, weight_b_a1, numCorrectPredictions,crossEntropys, cost, finalState, tf.no_op()], {
            input_data_c0: c[0],
            input_data_c1: c[1],
            input_data_es[0]: x[0],
            input_data_es[1]: x[1], 
        ...
        if (validateNumBatches > 10):
            f.write("b_e:")
            f.write(' '.join([str(content) for content in v_b]))
            f.write('\n')
            f.write("b_a:")
            f.write(' '.join([str(content) for content in v_b_a1]))
            f.write('\n')
    ...
    if (targetVocabulary==inputVocabulary):
        outputFile = open(
            "2012_ra/new_SuffixPrediction{0}Dataset{1}FLAGS.batch_size{2}unrollSteps{3}.txt".format(
