...
flags.DEFINE_string("traindir", "2017/new", "Directory to save checkpoints and gnuplot files.")
...
flags.DEFINE_integer("num_epochs_full_lr", 10,
                     "The number of epochs with full learning rate.")
flags.DEFINE_integer("num_epochs", 20, "The exact number of epochs.")
...
resultFile = open("2017/new_ResultFile_attention_allevents" + time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime()) + ".logging.csv", 'w')
...
with tf.Graph().as_default() as G:
    ...
    if (targetVocabulary==inputVocabulary):
        outputFile = open(
            "2017/new_SuffixPrediction{0}Dataset{1}FLAGS.batch_size{2}unrollSteps{3}.txt".format(
