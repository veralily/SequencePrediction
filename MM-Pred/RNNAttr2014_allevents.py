...
flags.DEFINE_string("traindir", "2014/new", "Directory to save checkpoints and gnuplot files.")
...
resultFile = open("2014/e0_ResultFile_attention_allevents" + time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime()) + ".logging.csv", 'w')
...
with tf.Graph().as_default() as G:
    ...
    for i in range(FLAGS.num_unroll_step):
      ...
      input_z = tf.reshape(tf.transpose(tf.concat([output_event,output_attr_1], -1), [1,0,2]), [output_event.get_shape()[1], -1])
    ...
    for i in range(FLAGS.num_unroll_step):
      output_event = tf.expand_dims(outputs_e0[i], -1)
      output_attr_1 = tf.expand_dims(outputs_e1[i], -1)
      input_z_a1 = tf.reshape(tf.transpose(tf.concat([output_event,output_attr_1], -1), [1,0,2]), [output_event.get_shape()[1], -1])
    ...
    # with tf.variable_scope("Attention_e", reuse=tf.AUTO_REUSE) as scope: 
    ...
    # the attention block for each cluster matrix
    # with tf.variable_scope("Attention_e") as scope: 
    #     inputs_c = tf.concat([input_c0, input_c1], 1)
    #     outputs_A = []
    #     for i in range(FLAGS.num_unroll_step):
    #         if i > 0: scope.reuse_variables()                       
    #         attention_bias = linear(inputs_c, FLAGS.hidden_size_A,
    #                             scope_name='attention_layer', reuse_scope=(i!=0))
    #         attention_output = tf.nn.sigmoid(linear(attention_bias, FLAGS.hidden_size_e0 + FLAGS.hidden_size_e1, scope_name = 'memory_output', reuse_scope=(i!=0)))
    #         print("attention_ouput.shape,{}".format(attention_output.get_shape()))

    #         outputs_A.append(attention_output)
    ...
    output_a = tf.reshape(tf.concat(outputs_2,1), [-1, FLAGS.hidden_size_2])
    ...
    for i in range(FLAGS.num_epochs):
        ...
        print("number of correct predictions: [%s]" %(' '.join([str(number) for number in accumNumCorrPred])))
        print("train precision: [%s]" %(' '.join([str(number) for number in thisPrecision])))
    ...
    for batchNum, (c, x, y) in enumerate(read.words_iterator(input_validation_traces, target_validation_traces, FLAGS.batch_size, FLAGS.num_unroll_step)):
        ...
        if (validateNumBatches > 10):
            if batchNum % (validateNumBatches // 10) == 10:
                ...
                print("valid precision: " + ' '.join([str(number / (iters * FLAGS.batch_size)) for number in accumNumCorrPred]))
    ...
    print("Dataset: [" + dataset + "/" + dataset + "] Epoch summary-- perplexity: %.3f speed: %.0f " %
        (numpy.exp(accumCosts / iters), iters * FLAGS.batch_size / (time.time() - start_time),))
    print("number of correct predictions: " + ' '.join([str(number) for number in accumNumCorrPred]))
    print("valid precision: " + ' '.join([str(number) for number in thisPrecision]))
    ...
    if (targetVocabulary==inputVocabulary):
        outputFile = open(
            "2014/new_SuffixPrediction{0}Dataset{1}FLAGS.batch_size{2}unrollSteps{3}.txt".format(
