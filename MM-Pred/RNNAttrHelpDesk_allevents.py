...
import os
import h5py
import readWordsHelpDesk as read
import string
...
flags.DEFINE_string("traindir", "HelpDesk_ra/helpdesk_new", "Directory to save checkpoints and gnuplot files.")
...
flags.DEFINE_integer("hidden_size_e3", 100, "Hidden size of lstm nn for e3.")
...
flags.DEFINE_integer("num_epochs_full_lr", 5,
                     "The number of epochs with full learning rate.")
flags.DEFINE_integer("num_epochs", 15, "The exact number of epochs.")
...
resultFile = open("HelpDesk/new_ResultFile_attention_allevents" + time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime()) + ".logging.csv", 'w')
...
with tf.Graph().as_default() as G:
    ...
    z = tf.concat([tf.multiply(outputs_e0[-1], outputs_e1[-1]),
        ...
        tf.multiply(outputs_e0[-1], outputs_e3[-1]),
        outputs_e0[-1],
        outputs_e1[-1],
        outputs_e2[-1],
        outputs_e3[-1]],
    ...
    with tf.variable_scope("modulator_e"):
        z_w = tf.get_variable("z_w", [z.get_shape()[1], 4], dtype=tf.float32)
        z_b = tf.get_variable("z_b", [4], dtype = tf.float32)
    ...
    for i in range(FLAGS.num_unroll_step):
      ...
      output_attr_3 = tf.expand_dims(outputs_e3[i], -1)
      input_z = tf.reshape(tf.transpose(tf.concat([output_event,output_attr_1,output_attr_2,output_attr_3], -1), [1,0,2]), 
    ...
    z = tf.concat([tf.multiply(outputs_e0[-1], outputs_e1[-1]),
        ...
        tf.multiply(outputs_e1[-1], outputs_e3[-1]),
        outputs_e0[-1],
        outputs_e1[-1],
        outputs_e2[-1],
        outputs_e3[-1]],
    ...
    for i in range(FLAGS.num_unroll_step):
      ...
      output_attr_2 = tf.expand_dims(outputs_e2[i], -1)
      output_attr_3 = tf.expand_dims(outputs_e3[i], -1)
      input_z_a1 = tf.reshape(tf.transpose(tf.concat([output_event,output_attr_1,output_attr_2,output_attr_3], -1), [1,0,2]), [output_event.get_shape()[1], -1])
    ...
    z = tf.concat([tf.multiply(outputs_e0[-1], outputs_e2[-1]),
        tf.multiply(outputs_e1[-1], outputs_e2[-1]),
        tf.multiply(outputs_e2[-1], outputs_e3[-1]),
        outputs_e0[-1],
        outputs_e1[-1],
        outputs_e2[-1],
        outputs_e3[-1]],
    ...
    for i in range(FLAGS.num_unroll_step):
      ...
      output_attr_2 = tf.expand_dims(outputs_e2[i], -1)
      output_attr_3 = tf.expand_dims(outputs_e3[i], -1)
      input_z_a2 = tf.reshape(tf.transpose(tf.concat([output_event,output_attr_1,output_attr_2,output_attr_3], -1), [1,0,2]), [output_event.get_shape()[1], -1])
    ...
    z = tf.concat([tf.multiply(outputs_e0[-1], outputs_e3[-1]),
        tf.multiply(outputs_e1[-1], outputs_e3[-1]),
        tf.multiply(outputs_e2[-1], outputs_e3[-1]),
        outputs_e0[-1],
        outputs_e1[-1],
    ...
    outputs_z_a3 = []
    variable_b_a3 = []
    with tf.variable_scope("modulator_a3"):
        z_w = tf.get_variable("z_w", [z.get_shape()[1], 4], dtype=tf.float32)
        z_b = tf.get_variable("z_b", [4], dtype = tf.float32)
        logits_z = tf.nn.xw_plus_b(z, z_w, z_b)
        b_a3 = tf.sigmoid(logits_z)
    for i in range(FLAGS.num_unroll_step):
      output_event = tf.expand_dims(outputs_e0[i], -1)
      output_attr_1 = tf.expand_dims(outputs_e1[i], -1)
      output_attr_2 = tf.expand_dims(outputs_e2[i], -1)
      output_attr_3 = tf.expand_dims(outputs_e3[i], -1)
      input_z_a3 = tf.reshape(tf.transpose(tf.concat([output_event,output_attr_1,output_attr_2,output_attr_3], -1), [1,0,2]), [output_event.get_shape()[1], -1])
      output_z_a3 = tf.transpose(tf.reshape(tf.multiply(tf.reshape(b_a3, [-1]), input_z_a3), [output_event.get_shape()[1], FLAGS.batch_size, -1]),[1,0,2])
      output_z_a3 = tf.reduce_sum(output_z_a3, 2)
      outputs_z_a3.append(output_z_a3)
      variable_b_a3.append(tf.expand_dims(tf.reduce_mean(b_a3,0),0))
    weight_b_a3 = tf.reduce_mean(tf.concat(variable_b_a3, 0),0)
    ...
    with tf.variable_scope("decoder4") as scope:
        # Create the individual LSTM layers
        lstm_cell = tf.contrib.rnn.BasicLSTMCell(FLAGS.hidden_size_2, forget_bias=0.0)
        # Add a propabilistic dropout to cells of the LSTM layer
        if (FLAGS.dropout_prob > 0):
        ...
        outputs_2 = []
        states_2 = []
        for i,input_ in enumerate(outputs_z_a3):
            #input_ = tf.concat([input_, output_c],1)
            if i > 0: scope.reuse_variables()
            input_ = tf.nn.relu(linear(input_, FLAGS.hidden_size_2,
                            scope_name='input_layer', reuse_scope=(i!=0)))
    ...
    output_a3 = tf.reshape(tf.concat(outputs_2,1), [-1, FLAGS.hidden_size_2])
    ...
    if (targetVocabulary==inputVocabulary):
        outputFile = open(
            "HelpDesk/new_SuffixPrediction{0}Dataset{1}FLAGS.batch_size{2}unrollSteps{3}.txt".format(
