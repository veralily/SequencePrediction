...
from CTES_model import CTESAModel
from CTES_model import CTESAInput
from CTES_model import FLAGS as FLAGS
...
def run_epoch(session, model, eval_op=None, verbose=False):
  ...
  fetches = {
      "cost": model.cost,
      "final_state_event": model.final_state_event,
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    with tf.name_scope("Train"):
      train_input = CTESAInput(config=config, data=train_data, name="TrainInput")
      with tf.variable_scope("Model", reuse=None, initializer=initializer):
        m = CTESAModel(is_training=True, config=config, input_=train_input)
      tf.summary.scalar("Training Loss", m.cost)
      tf.summary.scalar("Learning Rate", m.lr)

    with tf.name_scope("Valid"):
      valid_input = CTESAInput(config=config, data=valid_data, name="ValidInput")
      with tf.variable_scope("Model", reuse=True, initializer=initializer):
        mvalid = CTESAModel(is_training=False, config=config, input_=valid_input)
      tf.summary.scalar("Validation Loss", mvalid.cost)

    with tf.name_scope("Test"):
      test_input = CTESAInput(
          config=eval_config, data=test_data, name="TestInput")
      with tf.variable_scope("Model", reuse=True, initializer=initializer):
        mtest = CTESAModel(is_training=False, config=eval_config,
                         input_=test_input)
